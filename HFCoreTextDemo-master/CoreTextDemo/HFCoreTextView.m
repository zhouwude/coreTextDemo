...
- (void)drawRectWithLineByLineAlignment
{
    ...
    for (CFIndex i = 0; i < lineCount; i++)
    {
        ...
        // 微调Y值，需要注意的是CoreText的Y值是在baseLine处，而不是下方的descent。
        //self.font.descender为负值 lineDescent是正值
...
- (void)drawRectWithLineByLineAlignmentAndEllipses
{
    ...
    /*上篇博文已经实现了一行一行的绘制文本，但有些产品的需求是暂时显示一些简要的信息，用户点击后再进入到详情页显示完全。这时候则需要在文本的显示末端添加一个省略号。
     ...
     本次实现基本参照了Nimbus的实现，我按照自己的理解在代码中加入了一些注释，如果有不对的地方还请指出。*/
    // 1.创建需要绘制的文字
    NSMutableAttributedString *attributed = [[NSMutableAttributedString alloc] initWithString:self.text];
...
+ (CGFloat)textHeightWithText3:(NSString *)aText width:(CGFloat)aWidth font:(UIFont *)aFont
{
    ...
    CGSize suggestSize = CTFramesetterSuggestFrameSizeWithConstraints(framesetterRef, CFRangeMake(0, aText.length), NULL, CGSizeMake(aWidth, MAXFLOAT), NULL);//计算文本的size
